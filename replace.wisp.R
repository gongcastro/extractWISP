...
replace.wisp <-
  function(min.trials = 8,   # minimun number of successful test trials
    ...
    times.delete <- list()
    for (i in 1:n){
      times.filter[[i]] <- times[[i]] %>% dplyr::filter(time >= min.time, block == 1 | block == 2 | block == 3)
      times.extra[[i]] <- times[[i]] %>% dplyr::filter(block == 4 | block == 5)
      times.delete[[i]] <- times[[i]] %>% dplyr::filter(time < min.time, block == 1 | block == 2 | block == 3)
      }
